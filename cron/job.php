...
if($multi==1) {
	if(isset($_GET['multi']))
		...
		for($i=0;$i<=$xz;$i++){
			$start=$i*$szie;
			curl_run(($_SERVER['SERVER_PORT'] == '443' ? 'https://' : 'http://').$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?sys='.$sysid.'&key='.$_GET['key'].'&multi=on&start='.$start.'&num='.$szie);
		}
